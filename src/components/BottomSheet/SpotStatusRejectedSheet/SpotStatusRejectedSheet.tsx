          ...
          <BtnWrapper>
            ...
            }
            {
              items?.canRetrial && items?.rejectionType === 'INSUFFICIENCY' && items?.trialCount !== 3 &&
                ...
                <Button color={theme.black} backgroundColor={theme.white} border onClick={()=> handleSpotRetrial(items?.id!)}>오픈 재신청하기</Button>
            }
            {
              !items?.canRetrial && items?.rejectionType === 'INSUFFICIENCY' && items?.trialCount !== 3 &&
               // 트라이얼 재신청 조건 충족하지 못한 경우 false
                <Button color={theme.black} backgroundColor={theme.white} disabled border>30일 후 오픈 재신청이 가능합니다.</Button>
            }
          </BtnWrapper>
        </Content>
      </Wrapper>
